...
then
	...
	# aws --profile mine s3api list-objects --bucket s.natalian.org-inventory --prefix s.natalian.org/index/data/ --query 'sort_by(Contents, &LastModified)[-1].Key' --output text
...
zgrep $1 $today | tr -d '"' | while IFS=, read -r site path size date
do
	echo https://$site/$path
done
