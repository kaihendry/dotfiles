...
local function get_thermal()
  local temp, hint = "normal"

  if pcall(function ()
       local status
       local file=io.open(settings.temp_info, "r")
     ...
     then if temp >= settings.temperature_important then
             hint = "important" end
          if temp >= settings.temperature_critical then
	     hint = "critical" end
...
local function get_battery()
  ...
  if pcall(function ()
       local status
       local file=io.open(settings.bat_info, "r")
       local infocontents = file:read("*all")
       file:close();
  
       local file=io.open(settings.bat_state, "r")
       ...
       local percent = math.floor(remaining / lastfull * 100 + 0.5)
       local timeleft = " *AC*"
       local hours, secs, mins
       if get_ac() == 0 and rate > 0 then
       ...
       if secs ~= nil and secs <= settings.batterytimeleft_important then timelefthint = "important" end 
       if secs ~= nil and secs <= settings.batterytimeleft_critical then timelefthint = "critical" end
       if percent <= settings.batterypercent_important then percenthint = "important" end
       if percent <= settings.batterypercent_critical then percenthint = "critical" end
...
local function update_laptopstatus ()
  ...
  if statusd ~= nil then
    ...
    laptopstatus_timer:set(settings.interval*1000, update_laptopstatus)
  else
